...
function getDllsForDirectXVersion(directXVersion) {
  ...
  console.log(
    `Normalized DirectX version: "${normalizedVersion}" -> Primary version: ${primaryVersion}`
  );
  ...
  if (
    primaryVersion === 8 ||
    normalizedVersion.includes('direct3d 8') ||
    normalizedVersion.includes('d3d8')
  ) {
    result.requiredDlls.push('d3d8.dll', 'd3d9.dll');
    result.description = 'Direct3D 8';
  } else if (
    primaryVersion === 9 ||
    normalizedVersion.includes('direct3d 9') ||
    normalizedVersion.includes('d3d9')
  ) {
    result.requiredDlls.push('d3d9.dll');
    result.description = 'Direct3D 9';
  } else if (
    primaryVersion === 10 ||
    normalizedVersion.includes('direct3d 10') ||
    normalizedVersion.includes('d3d10')
  ) {
    result.requiredDlls.push('dxgi.dll', 'd3d11.dll', 'd3d10core.dll');
    result.description = 'Direct3D 10';
  } else if (
    primaryVersion === 11 ||
    normalizedVersion.includes('direct3d 11') ||
    normalizedVersion.includes('d3d11')
  ) {
    result.requiredDlls.push('dxgi.dll', 'd3d11.dll');
    result.description = 'Direct3D 11';
  } else if (
    primaryVersion === 12 ||
    normalizedVersion.includes('direct3d 12') ||
    normalizedVersion.includes('d3d12')
  ) {
...
async function applyDxvkToGame(game, metadata, dxvkType, version, cacheDir) {
  try {
    ...
    const { existingDlls } = checkExistingDlls(gameDir, requiredDlls);
    ...
    // Copy DXVK DLLs to the game directory
    const copied = await copyDxvkDlls(
      sourceDxvkDir,
      archSubfolder,
      gameDir,
      requiredDlls,
    ...
    try {
      if (typeof global.saveGameMetadata === 'function') {
        ...
        console.warn(
          'saveGameMetadata function not available, metadata changes will not be persisted'
        );
      }
    } catch (error) {
...
function getInstalledDxvkVersions(appDataPath) {
  ...
  try {
    ...
    if (fs.existsSync(dxvkCacheDir)) {
      result.dxvk = fs.readdirSync(dxvkCacheDir).filter(dir => {
    ...
    if (fs.existsSync(gplasyncCacheDir)) {
      result.dxvkGplasync = fs.readdirSync(gplasyncCacheDir).filter(dir => {
