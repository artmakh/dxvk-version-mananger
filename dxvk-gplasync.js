...
async function fetchGplasyncReleases() {
  try {
    ...
    const apiUrl = 'https://gitlab.com/api/v4/projects/Ph42oN%2Fdxvk-gplasync/repository/tree?path=releases&ref=main';
            ...
            if (Array.isArray(response)) {
              ...
              const releases = response
                ...
                .map(item => {
                  ...
                  const versionMatch = item.name.match(/dxvk-gplasync-([0-9.]+(?:-[0-9]+)?)\.tar\.gz/);
  ...
  return new Promise((resolve, reject) => {
    ...
    function verifyExtraction() {
      ...
      try {
        ...
        if (items.length > 0) {
          console.log(`Extraction appears successful despite errors. Found ${items.length} items in the directory.`);
